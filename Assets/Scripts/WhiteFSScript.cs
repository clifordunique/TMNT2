...
public class WhiteFSScript : MonoBehaviour {
	public int life = 2;
	public int gothit = 0;
	...
	void Start () {
		...
		attackCollider = this.transform.FindChild("WFSAttColl").GetComponent<BoxCollider2D>();
		jumpColl = this.transform.FindChild("WFSJumpColl").GetComponent<BoxCollider2D>();
	...
	void Update () {
		...
		if(attacking)
		{
			if (animator.GetCurrentAnimatorStateInfo(0).IsName("WhiteFSPunchingEnd")){
				attacking = false;
				animator.SetBool ("Attacking", false);
				attackCollider.enabled = false;
			}
